...
device = args.device
res_path = os.path.join('./datasets', task_name_dict[task_name], 'acc.json')
...
DataLoader = DataLoader[task_name]
...
for seed in [8, 13, 42, 50, 60]:
    ...
    data_bundle = DataLoader(tokenizer=tokenizer, n_prompt_tokens=n_prompt_tokens).my_load(['test'], seed)
    ...
    with open(res_path, 'a', encoding='utf-8') as f:
        f.write(json.dumps({'seed':seed, 'acc':acc.item()}))
        f.write('\n')
